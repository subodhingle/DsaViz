...
import { LayoutTemplate, Columns2, Code2, ArrowLeft, Hexagon, Menu, X } from 'lucide-react'
...
export default function Navbar({ onMenuClick, sidebarOpen }) {
  ...
  return (
    <motion.nav
      ...
      className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-3 sm:px-5 h-12"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => navigate('/')} className="flex items-center gap-2 group">
          ...
          <span className="text-sm font-semibold tracking-tight hidden xs:block" style={{ color: 'var(--text-primary)' }}>
            DSA<span style={{ color: 'var(--accent)' }}>Viz</span>
          </span>
          {isApp && (
            <span className="hidden sm:flex items-center gap-1 text-xs ml-1"
              style={{ color: 'var(--text-muted)' }}>
      ...
      {/* ── Center: view toggle (app only) ── */}
      {isApp ? (
        <div className="flex items-center gap-0.5 rounded-lg p-0.5"
          style={{ background: 'var(--canvas)', border: '1px solid var(--border-subtle)' }}>
          {VIEWS.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setView(id)}
              className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 rounded-md text-xs font-medium transition-all duration-150"
              style={view === id
        ...
        <button onClick={() => navigate('/app')}
          className="btn-accent text-xs px-3 sm:px-4 py-1.5 rounded-lg">
          Launch App
        </button>
      )}
      ...
      {/* ── Right: theme dots ── */}
      <div className="flex items-center gap-2">
        <span className="text-xs hidden sm:block" style={{ color: 'var(--text-disabled)' }}>Theme</span>
